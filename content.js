...
const updateAttributes = (al, bl) => {
    document.documentElement.dataset.nmtAllowlist = JSON.stringify(al);
    document.documentElement.dataset.nmtBlacklist = JSON.stringify(bl);
};
...
const showPopup = (url, host) => {
    if (window !== window.top) {
        window.top.postMessage({ action: "NMT_IFRAME", url }, "*");
        return;
    }
    ...
    shadow.innerHTML = `
        <style>
            .ov { position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(0,0,0,0.5);backdrop-filter:blur(2px);display:flex;align-items:center;justify-content:center;font-family:sans-serif; }
            .cd { 
                background:#fff;width:400px;padding:24px;border-radius:12px;
                box-shadow:0 10px 40px rgba(0,0,0,0.4); 
                animation: popIn 0.2s cubic-bezier(0.16, 1, 0.3, 1);
            }
            .hd { display:flex;align-items:center;gap:10px;margin-bottom:15px; }
            .hd img { width:24px;height:24px; }
            .hd h3 { margin:0;font-size:16px;color:#111; }
            .u { background:#f8fafc;padding:12px;border-radius:6px;font-size:13px;margin-bottom:16px;word-break:break-all;color:#334155;border:1px solid #e2e8f0;line-height:1.4; }
            ...
            button { 
                ...
                font-weight:600;font-size:14px;background:#1e293b;color:#fff;
    ...
    shadow.getElementById("c").onclick = () => {
        if (alCb.checked) {
            chrome.storage.sync.get(["allowlist", "blacklist"], (data) => {
                ...
                if (!al.includes(host)) {
                    const newList = [...al, host];
                    chrome.storage.sync.set({ 
                        allowlist: newList,
                        blacklist: bl.filter(i => i !== host)
                    });
                    // Immediate Update
                    loadLists().then(({ al, bl }) => updateAttributes(al, bl));
                }
            });
        }
    ...
    shadow.getElementById("b").onclick = () => {
        if (blCb.checked) {
            chrome.storage.sync.get(["allowlist", "blacklist"], (data) => {
                ...
                let bl = data.blacklist || [];
                if (!bl.includes(host)) {
                    const newList = [...bl, host];
                    chrome.storage.sync.set({ 
                        blacklist: newList,
                        allowlist: al.filter(i => i !== host)
                    });
                    // Immediate Update
                    loadLists().then(({ al, bl }) => updateAttributes(al, bl));
                }
...
loadLists().then(({ al, bl }) => {
    updateAttributes(al, bl);
    injectScript();
});
...
window.addEventListener("NMT_BLOCKED", e => {
    showPopup(e.detail.url, getHost(e.detail.url));
});
